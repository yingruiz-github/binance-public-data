...
def main():
    ...
    for pair in TARGET_PARIS:
        ...
        # monthly 
        #print("runned")
        monthly_cmd = f"conda run -n {CONDA_ENV_NAME} python download-trade.py -t spot -s {pair} -c 1 -skip-daily 1".split() # {CONDA_RUN_PREFIX} 
        ...
        daily_cmd = f"conda run -n {CONDA_ENV_NAME} python download-trade.py -t spot -s {pair} -c 1 -skip-monthly 1 -startDate \
